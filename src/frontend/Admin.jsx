...
function Admin({handleShowToApprove, prevSubMenu=null, close, addPopup}) {
    ...
    const setSubPanel = (num, bookData = null) => {
        switch (num) {
            case 1:
                addPopup(
                    <AddBook onDone={() => {doQuery(); setSubPanel(0);}} />
,
                    '30rem', '70%'
                );
                break;
            case 2:
                addPopup(
                    <div className="admin_book_editing">
                        <div className='admin_book_add'><AddBook bookData={bookData} onDone={() => {doQuery(); setSubPanel(0);}} /></div>
                        <div className='admin_book_add'><EditCopies addPopup={addPopup} id_book={bookData.id_libro} onDone={() => {doQuery(); setSubPanel(0);}} /></div>
                    </div>,
                    '60rem', '70%'
                );
        }
        ...
        <>
            ...
            <div className='books_content'>
                {subMenu === "users" && data? data.map((user, index) => (
                    <Box key={index} onClick={() => handleShowToApprove(user)} borderColor={user.pendiente === "si" ? "var(--color-6)" : "var(--color-1)"} className="book">
                        <div>
                            <h1>{user.nombres}</h1>
                            <p>{user.correo}</p>
